...
class lj_repulsion:
    ...
    def repulse(self, all_particles, x=800, y=600):
        ...
        compute_in_group(grouped_objects1.values(), len(all_particles), bins=self.bins, collision_force=self.factor)
        
        sorted_parts2 = sorted(all_particles, key=lambda particle: particle.bin_number2)
        grouped_objects2 = {key: list(group) for key, group in groupby(sorted_parts2, key=lambda x: x.bin_number2)}
        compute_in_group(grouped_objects2, len(all_particles), bins=self.bins, collision_force=self.factor)

                

def compute_in_group(groups, N, bins = 16, collision_force = 80000.):
    for group in groups:
            ...
            for index2, part2 in enumerate(group):
                if index1 > index2:
                    ...
                    if part.mass > 10 or part2.mass > 10:
                        ...
                        norme = 4
                    intensity = part.mass * part2.mass * collision_force / norme ** 2
