...
class lj_repulsion:
    ...
    def repulse(self, all_particles, x=800, y=600):
        ...
        if multi:
            self.pool.starmap(compute_in_group, [(group, len(all_particles), self.bins,self.factor) for group in grouped_objects.values()])
        else:
            for group in grouped_objects.values():
                compute_in_group(group, len(all_particles), bins = self.bins, collision_force= self.factor)

def compute_in_group(group, N, bins = 100, collision_force = 80000.):
